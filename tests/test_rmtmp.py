...
def init_tmpdir():
    ...
    touch_files = ['testfile', 'testdir/testfile2',
                   'ignorefile', 'ignoredir/testfile3']
    for d in mkdir_dirs:
        Path(tmpdir, d).mkdir()
...
class TestRmTmp():
    ...
    def test_run(self, init_tmpdir):
        ...
        assert set(files) == set([Path(tmpdir, '.rmtmpignore'),
    ...
    def test_get_files(self, init_tmpdir):
        ...
        expect_paths = [Path(tmpdir, '.rmtmpignore'),
                        Path(tmpdir, 'testfile'), Path(tmpdir, 'ignorefile'),
                        Path(tmpdir, 'testdir'), Path(tmpdir, 'ignoredir')]
        assert set(files) == set(expect_paths)

    def test_filter_files(self, init_tmpdir):
        tmpdir = init_tmpdir
        target_paths = [Path(tmpdir, '.rmtmpignore'),
                        Path(tmpdir, 'testfile'), Path(tmpdir, 'ignorefile'),
                        Path(tmpdir, 'testdir'), Path(tmpdir, 'ignoredir')]
